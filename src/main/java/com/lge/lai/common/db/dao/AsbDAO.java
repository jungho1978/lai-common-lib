...
public class AsbDAO extends BaseDAO implements DaoCallback {
	...
	private static final String SQL_DELETE_BY_ID = "DELETE FROM " + TABLE
			+ " WHERE _id = ?";
	...
	public long create(Object obj) throws DAOException {
		if (obj instanceof Asb) {
			Asb asb = (Asb) obj;
			Object[] values = { asb.versionName, asb.type, asb.desc,
					asb.packageName, asb.className, asb.actionName,
					asb.updatedBy, DAOUtil.toSqlTimestamp() };
	...
	public Object convertToDTO(ResultSet rs) throws SQLException {
		...
		return new Asb(version, type, desc, packageName, className, actionName,
				updatedBy, id);
	}
}

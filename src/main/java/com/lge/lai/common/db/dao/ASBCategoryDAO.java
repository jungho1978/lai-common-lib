...
public class ASBCategoryDAO extends BaseDAO implements DaoCallback {
    ...
    private static final String SQL_INSERT = "INSERT INTO " + TABLE + " " + "(_asb_id, _category) "
            + "VALUES " + "(?, ?)";
    ...
    private static final String SQL_LIST_BY_FK = "SELECT * FROM (SELECT * FROM "
            + ASB_TABLE + ") AS ASB_T INNER JOIN (SELECT * FROM " + TABLE
            + " WHERE _asb_id = ?) AS ASB_CATEGORY_T ON ASB_T._id = ASB_CATEGORY_T._asb_id";

    private static final String SQL_LIST_BY_FK_AND_CATEGORY = "SELECT * FROM (SELECT * FROM "
            + ASB_TABLE + ") AS ASB_T INNER JOIN (SELECT * FROM " + TABLE
    ...
    public List<Object> list(String category) throws DAOException {
    ...
    public Object convertToDTO(ResultSet rs) throws SQLException {
        ...
        return new ASBCategory(version, type, desc, packageName, className, actionName, category,
                updatedBy, id);
    }
}
